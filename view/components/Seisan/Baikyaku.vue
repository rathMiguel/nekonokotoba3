...
<template>
  <section>
    <header class="mb-4">
      ...
      <p>
        <small
          ...
          />ボタンで入力枠が増やせます。分割で売却した時の計算にお使いください！</small
        >
      </p>
    </header>
    <div class="flex items-center mb-2" v-for="(value, index) in seisanStatus.amounts" ref="amountRefs">
      <input
        type="number"
        class="w-60 border-2 rounded-lg p-2 border-primary"
        v-on:keydown.enter="handleRowButton(index).increment"
        v-model="seisanStatus.amounts[index]"
      />
      <button
        title="入力枠を増やす"
        class="rounded-md size-11 text-xl ml-2"
        v-on:click="handleRowButton(index).increment"
      >
        <font-awesome-icon class="icon" :icon="['fas', 'circle-plus']" />
      </button>
      <button
        title="入力枠を減らす"
        class="rounded-md size-11 text-xl"
        v-if="seisanStatus !== undefined && seisanStatus.amounts.length - 1 > 0"
        v-on:click="handleRowButton(index).decrement"
      >
